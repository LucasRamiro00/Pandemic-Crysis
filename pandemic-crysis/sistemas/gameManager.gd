...
const LIMITE_MAXIMO_LEITOS: int = 14
const BONUS_MINIMO_EXAME: float = 0.5

func upgrade_disponivel(tipo_upgrade: String) -> bool:
	match tipo_upgrade:
		"ampliar_leitos":
			return limite_de_leitos < LIMITE_MAXIMO_LEITOS
		"treinar_funcionarios":
			return bonus_velocidade_exame > BONUS_MINIMO_EXAME
		"modernizar_terminal":
			return not terminal_modernizado
	return false

func comprar_upgrade(tipo_upgrade: String, custo: int) -> bool:
	if not upgrade_disponivel(tipo_upgrade):
		return false
	if caixa_hospital >= custo:
		caixa_hospital -= custo
		match tipo_upgrade:
			"ampliar_leitos":
				limite_de_leitos = min(limite_de_leitos + 3, LIMITE_MAXIMO_LEITOS)
			"treinar_funcionarios":
				bonus_velocidade_exame = max(bonus_velocidade_exame - 0.1, BONUS_MINIMO_EXAME)
			"modernizar_terminal":
...
func realizar_fechamento_de_caixa() -> void:
	...
	relatorio_do_dia = {
		...
		"manutencao": custo_manutencao_diaria,
		"meta": meta_pacientes_dia,
		"falhas_seguidas": dias_consecutivos_falhando_quota
	}
